...
class WasmtimeIntegrationTest {
    ...
    void voidFunction() {
        try (Engine engine = WasmtimeEngineAdapter.create(null);
             Module module = engine.loadModule(VOID_MODULE)) {
            Instance instance = module.instantiate();
    ...
    void memoryDirectAccess() {
        try (Engine engine = WasmtimeEngineAdapter.create(null);
            ...
            memory.write(0, data);
            byte[] readBack = memory.read(0, data.length);
    ...
    @Test
    @EnabledIf("runtimeAvailable")
    void fuelControllerAddAndQuery() {
        ...
        try (Engine engine = WasmtimeEngineAdapter.create(config);
             Module module = engine.loadModule(ADD_MODULE)) {
            FuelController fuel = module.extension(FuelController.class).orElseThrow();
            assertEquals(0, fuel.fuelRemaining());
            fuel.addFuel(10000);
            assertEquals(10000, fuel.fuelRemaining());

            Instance instance = module.instantiate();
            Function add = instance.function("add").orElseThrow();
            assertEquals(7, add.invoke(3, 4));

            assertTrue(fuel.fuelRemaining() < 10000);
        }
    }

    @Test
    @EnabledIf("runtimeAvailable")
    void fuelControllerNotAvailableWithoutConfig() {
        try (Engine engine = WasmtimeEngineAdapter.create(null);
             Module module = engine.loadModule(ADD_MODULE)) {
            assertFalse(module.extension(FuelController.class).isPresent());
        }
    }
}
