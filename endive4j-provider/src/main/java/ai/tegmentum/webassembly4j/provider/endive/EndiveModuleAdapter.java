...
final class EndiveModuleAdapter implements Module {

    private final WasmModule wasmModule;
    private final java.util.function.Function<WasmModule, run.endive.runtime.Instance.Builder> builderFactory;

    EndiveModuleAdapter(WasmModule wasmModule,
                         java.util.function.Function<WasmModule, run.endive.runtime.Instance.Builder> builderFactory) {
    ...
    public Instance instantiate() {
        try {
            run.endive.runtime.Instance nativeInstance =
                    builderFactory.apply(wasmModule).build();
            return new EndiveInstanceAdapter(nativeInstance);
        } catch (Exception e) {
    ...
    public Instance instantiate(LinkingContext linkingContext) {
        ...
        try {
            ...
            for (HostFunctionDefinition def : hostFunctions) {
                List<ValType> paramTypes = convertToEndiveTypes(def.parameterTypes());
                List<ValType> returnTypes = convertToEndiveTypes(def.resultTypes());
    ...
    private static List<ValType> convertToEndiveTypes(ValueType[] types) {
        List<ValType> result = new ArrayList<>(types.length);
        for (ValueType type : types) {
            result.add(convertToEndiveType(type));
        }
        return result;
    }

    private static ValType convertToEndiveType(ValueType type) {
        switch (type) {
    ...
    private ExportDescriptor convertFunctionExport(Export export) {
        ...
        if (localIndex < wasmModule.functionSection().functionCount()) {
            ...
            return ExportDescriptor.function(export.name(),
                    convertEndiveTypes(funcType.params()),
                    convertEndiveTypes(funcType.returns()));
        }
        return ExportDescriptor.function(export.name(),
                new ValueType[0], new ValueType[0]);
    }
    ...
    private static ValueType[] convertEndiveTypes(List<ValType> types) {
        ValueType[] result = new ValueType[types.size()];
        for (int i = 0; i < types.size(); i++) {
            result[i] = convertEndiveType(types.get(i));
        }
        return result;
    }

    private static ValueType convertEndiveType(ValType type) {
