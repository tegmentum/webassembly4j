...
/**
 * Endive implementation of the GC extension.
 *
 * <p>Endive GC objects are plain Java objects allocated on the JVM heap.
 * Values are encoded as {@code long} using Endive's {@link Value} helpers.
 ...
 */
final class EndiveGcExtension implements GcExtension {
    ...
    public GcStructInstance createStruct(GcStructType type, GcValue... values) {
        ...
        return new EndiveGcStructInstance(nativeStruct, type);
    }
    ...
    public GcArrayInstance createArray(GcArrayType type, GcValue... elements) {
        ...
        allocationCount.incrementAndGet();
        return new EndiveGcArrayInstance(nativeArray, type);
    }
    ...
    public GcI31Instance createI31(int value) {
        ...
        allocationCount.incrementAndGet();
        return new EndiveGcI31Instance(nativeRef);
    }
    ...
    private static final class EndiveGcStructInstance implements GcStructInstance {

        private final WasmStruct nativeStruct;
        private final GcStructType structType;

        EndiveGcStructInstance(WasmStruct nativeStruct, GcStructType structType) {
        ...
        public boolean refEquals(GcObject other) {
            if (!(other instanceof EndiveGcStructInstance)) {
                return false;
            }
            return this.nativeStruct == ((EndiveGcStructInstance) other).nativeStruct;
        }
    }

    private static final class EndiveGcArrayInstance implements GcArrayInstance {

        private final WasmArray nativeArray;
        private final GcArrayType arrayType;

        EndiveGcArrayInstance(WasmArray nativeArray, GcArrayType arrayType) {
        ...
        @Override
        public boolean refEquals(GcObject other) {
            if (!(other instanceof EndiveGcArrayInstance)) {
                return false;
            }
            return this.nativeArray == ((EndiveGcArrayInstance) other).nativeArray;
        }
    }

    private static final class EndiveGcI31Instance implements GcI31Instance {

        private final WasmI31Ref nativeRef;

        EndiveGcI31Instance(WasmI31Ref nativeRef) {
        ...
        @Override
        public boolean refEquals(GcObject other) {
            if (!(other instanceof EndiveGcI31Instance)) {
                return false;
            }
            return this.nativeRef.value() == ((EndiveGcI31Instance) other).nativeRef.value();
        }
    }
}
